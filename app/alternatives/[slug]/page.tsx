...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){
  ...
  const description=`Compare ${item.competitor} alternatives by equivalent hours, location, management, backup, contract terms, and current scope and contract terms.`;
...
export default async function AlternativePage({params}:{params:Promise<{slug:string}>}){
  ...
  const sections=[
    ...
    ['scope','Scope and contract check'],
    ['decision','A practical decision routine'],
  ];
  const faq=[
    [`What is the best ${item.competitor} alternative?`,`There is no universal winner. The best fit is the provider that meets the same written task scope, hours, location, management, and risk controls with terms your team can manage.`],
    [`How should I compare ${item.competitor} proposals?`,`Convert every proposal to the same monthly capacity, then compare setup, software, management, backup, overage rules, and contract terms. Do not compare a small hour bundle with a full-time hire.`],
  ...
  return <><Header/><main className="alt-page"><JsonLd data={schema}/><JsonLd data={faqSchema}/>
    <header className="alt-hero"><div className="container alt-hero-grid"><div><p className="eyebrow">Independent comparison · Scope checked July 29, 2026</p><h1>{item.competitor} Alternatives: A Fair 2026 Comparison</h1><p className="lead">Compare the same work, capacity, support, and risk controls before treating one provider as a substitute for another.</p><div className="alt-hero-tags"><span>{item.serviceModel}</span><span>{item.scopeSignal}</span></div></div><img src={item.image} width="620" height="410" alt={`${item.competitor} alternatives buyer comparison`}/></div></header>
