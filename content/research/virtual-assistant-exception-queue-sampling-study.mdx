...
excerpt: Evidence-led research on exception-queue sampling for bounded and reviewable virtual assistant services.
...
heroImageAlt: evidence review for exception-queue sampling
...
This question extends the site's [work-intake triage research](/research/virtual-assistant-work-intake-triage) and [service quality assurance research](/research/virtual-assistant-service-quality-assurance) by concentrating on the cases that leave the normal path.

## Method and evidence scope

The proposed unit is one exception record tied to its original task, rule, decision owner, timestamps, and final resolution. Select a fixed review period before looking at outcomes. Draw cases from the full eligible set, retain exclusions, and prevent the person who performed the work from silently choosing only clean examples. A second reviewer applies the same definitions without seeing the assistant's preferred conclusion.
...
An inbox assistant routes messages that fall outside approved reply rules. The queue includes genuine judgment calls, missing customer context, access failures, and ordinary messages misclassified as exceptions. The record should preserve the original input, the applicable rule, the assistant's action, any question sent to the owner, and the final disposition. Without that chain, reviewers may judge a reconstructed story instead of the decision that occurred.
...
The queue excludes routine work that never triggered a flag, so its error mix cannot describe overall assistant performance. Urgent cases may also be overrepresented because managers resolve and document them first. Work may also change because people know it is being reviewed. That does not invalidate the exercise, but it limits any claim about normal practice.
...
A stratified sample is useful for redesigning escalation rules when the strata, exclusions, and unresolved cases remain visible. It is not a substitute for a workload-wide quality measure. Report the numerator and denominator, the cases excluded, the first-pass reviewer differences, and any unresolved items. A percentage without those details invites a precision the study did not earn.
...
The evidence supports a bounded conclusion about exception-queue sampling: define the observation unit before sampling, retain difficult and unresolved cases, compare independent judgments, and keep interpretation with an accountable owner. That approach gives a buyer inspectable evidence about the workflow without claiming more than the records show.
