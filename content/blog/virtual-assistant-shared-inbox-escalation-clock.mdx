---
slug: virtual-assistant-shared-inbox-escalation-clock
title: Set an Escalation Clock for a Shared Inbox
excerpt: Make urgent messages visible without treating every new email as an emergency.
...
heroImageAlt: Set an Escalation Clock for a Shared Inbox workflow evidence
...
---
# Set an Escalation Clock for a Shared Inbox

A shared inbox escalation clock gives a virtual assistant a bounded operating check while an accountable manager retains judgment, approval, and exception authority.

## Set an Escalation Clock for a Shared Inbox: start with a defined record

Define urgency labels with concrete examples, the maximum waiting time for each label, the person who can decide exceptions, and the evidence required in an escalation. State the source and verification time for each field. Keep missing information visible instead of guessing, and align status labels with the [project coordination checklist](/blog/virtual-assistant-project-coordination-checklist).
