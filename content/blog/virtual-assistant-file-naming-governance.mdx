---
slug: virtual-assistant-file-naming-governance
title: A File-Naming Rule That Supports Reliable Handoffs
excerpt: Use stable names and versions so the next reviewer can find the right record.
...
heroImageAlt: A File-Naming Rule That Supports Reliable Handoffs workflow evidence
...
---
# A File-Naming Rule That Supports Reliable Handoffs

A file-naming governance rule gives a virtual assistant a bounded operating check while an accountable manager retains judgment, approval, and exception authority.

## A File-Naming Rule That Supports Reliable Handoffs: start with a defined record

Define required fields such as project, document type, date, status, and version. Prohibit ambiguous labels such as final-final and keep the authoritative file in one approved location. State the source and verification time for each field. Keep missing information visible instead of guessing, and align status labels with the [project coordination checklist](/blog/virtual-assistant-project-coordination-checklist).
