---
slug: virtual-assistant-article-filename-convention
title: Virtual Assistant Article Filename Convention
excerpt: A naming routine that keeps briefs, drafts, evidence, and approved versions easy to identify.
...
---
# Virtual Assistant Article Filename Convention

Published September 2, 2026.

A filename convention reduces version confusion during a fast daily publishing cycle. This routine supports proper daily article creation by making the file identity visible before the work moves forward. It gives a virtual assistant a useful operating boundary while leaving editorial judgment with the accountable owner.
...
## Virtual Assistant Article Filename Convention: establish the control

Use fields the team can verify, such as slug, document type, status, and revision number. Keep the public date in metadata rather than relying on a file's creation timestamp as release evidence. Record the result in the shared article record, with a timestamp and the person responsible for the next action. The record should show facts and decisions separately.
...
Place the article filename convention at a named transition in the article workflow. Define what must be present before the check starts, what counts as passing, and where a failed item goes. A clear correction path prevents the same incomplete package from circulating between people.
...
Close the article filename convention with the current status, evidence reviewed, unresolved point, next action, owner, and due time. Use plain status language such as ready for editorial review, held for source confirmation, returned for correction, or published and verified.
